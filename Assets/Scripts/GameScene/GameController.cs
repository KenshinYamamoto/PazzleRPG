...
public class GameController : MonoBehaviour
{
    ...
    [SerializeField] List<DropController> removeDrops = new List<DropController>(); // 削除するドロップを把握する
    [SerializeField] GameObject menuButton;
    DropController currentDraggingDrop;
    public List<DropController> drops = new List<DropController>(); // 盤面にある全てのドロップを把握する
    ...
    void OnDragging()
    {
        ...
        if (hit && hit.collider.GetComponent<DropController>())
        {
            ...
            if (drop.id == currentDraggingDrop.id) // 同じ種類だったら
            {
                float distance = Vector2.Distance(drop.transform.position, currentDraggingDrop.transform.position);
                if (distance < ParamsSO.Entity.dropRemoveDistance) // 距離が近かったら
                {
                    AddRemoveDrop(drop);
                }
            }
